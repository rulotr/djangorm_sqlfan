...
def agregar_primer_detalle():
    libro = Libro.objects.filter(isbn='1933988495').update(
    detalles = {'genero': [
        'Programming', 'Python'], 'idioma': 'Ingles', 'Disponible': None, 
        'Estrellas': 5})

    Libro.objects.filter(isbn='132632942').update(
        detalles={'genero': ["Java", "Business"],'idioma': 'Español', 
                 'Disponible': 3})
